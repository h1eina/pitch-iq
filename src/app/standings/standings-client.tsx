...
import type { ApiStanding } from '@/lib/football-api';
...
function parseForm(formStr: string | null): FormResult[] {
  if (!formStr) return [];
...
export default function StandingsClient({ standingsByLeague }: { standingsByLeague: Record<string, ApiStanding[]> }) {
  const [leagueId, setLeagueId] = useState('premier-league');
  const [sortKey, setSortKey] = useState<'points' | 'goalDifference' | 'goalsFor'>('points');

  const standings = useMemo(() => {
    const s = standingsByLeague[leagueId] ?? [];
    return [...s].sort((a, b) => {
      if (sortKey === 'points') return b.points - a.points;
      if (sortKey === 'goalDifference') return b.goalDifference - a.goalDifference;
      return b.goalsFor - a.goalsFor;
    });
  }, [leagueId, sortKey, standingsByLeague]);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Real standings with form and projections</p>
        </div>
      ...
      <div className="flex gap-2">
        {(['points', 'goalDifference', 'goalsFor'] as const).map(k => (
          <button key={k} onClick={() => setSortKey(k)}
            ...
            {k === 'goalDifference' ? 'GD' : k === 'goalsFor' ? 'GF' : 'Pts'}
          </button>
        ))}
      </div>
      ...
      {standings.length === 0 ? (
        <div className="card p-8 text-center text-slate-400">
          <p className="text-lg font-bold">No standings data available</p>
          <p className="text-sm mt-1">API data may be loading or unavailable</p>
        </div>
      ) : (
                  ...
                  <motion.tr key={s.team.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-white text-xs">{s.team.shortName || s.team.name}</span>
                      </div>
                    </td>
                    <td className="px-2 py-2 text-center text-slate-400">{s.playedGames}</td>
                    <td className="px-2 py-2 text-center text-emerald-400">{s.won}</td>
                    <td className="px-2 py-2 text-center text-amber-400">{s.draw}</td>
                    ...
                    <td className="px-3 py-2 hidden lg:table-cell">
                      <div className="flex gap-0.5 justify-center">{form.slice(-5).map((f, fi) => <FormBadge key={fi} r={f} />)}</div>
                    </td>
